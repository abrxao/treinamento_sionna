...
while traci.simulation.getMinExpectedNumber() > 0:
        ...
        for i in range(0,len(vehicles)):
                ...
                vehList = [getdatetime(),steps, vehid, coord, gpscoord, spd, edge, lane, displacement, turnAngle, nextTLS]
                ...
                for k in range(0,len(trafficlights)):
                        ...
                        if idd in traci.trafficlight.getControlledLanes(trafficlights[k]):
                                ...
                                tl_program = traci.trafficlight.getAllProgramLogics(trafficlights[k])
                                tl_next_switch = traci.trafficlight.getNextSwitch(trafficlights[k])

                                #Packing of all the data for export to CSV/XLSX
                                tlsList = [steps,tflight, tl_state, tl_phase_duration, tl_lanes_controlled, tl_program, tl_next_switch]
                                ...
                                """ print(trafficlights[k], " --->", \
                                      ...
                                      #Returns the complete traffic light program, structure described under data types                                      
                                        " TLS Program: ", traci.trafficlight.getAllProgramLogics(trafficlights[k]), " |"
